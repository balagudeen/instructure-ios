...
public struct Scope {
    ...
    public static func `where`(_ key: String, equals value: Any, orderBy order: String? = nil, ascending: Bool = true) -> Scope {
        let predicate = NSPredicate(format: "%K == %@", argumentArray: [key, value])
        let sort = NSSortDescriptor(key: order ?? key, ascending: ascending)
        return Scope(predicate: predicate, order: [sort])
    }

    public static func all(orderBy order: String) -> Scope {
