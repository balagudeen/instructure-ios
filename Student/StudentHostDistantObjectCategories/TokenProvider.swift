...
class TokenProvider: NSObject {
    ...
    var webView: WKWebView {
        ...
        let tag = 42
        guard let webView = UIApplication.shared.keyWindow!.subviews[0] as? WKWebView, webView.tag == tag else {
            let view = WKWebView(frame: UIScreen.main.bounds)
            view.tag = tag
            view.navigationDelegate = self

            // The web view must be on screen in order for it to make requests
            // Inserts the web view below the root nav so that it's not visible
